// Configuration
const ESP_IP = window.location.hostname; // Use current hostname (same device)
...
function updateServoValues(prefix, data) {
  ...
  if (Array.isArray(servosArray)) {
    for (let i = 0; i < servosArray.length; ++i) {
      ...
      if (currentServo) currentServo.value = servosArray[i].current;
      if (currentServoValue) currentServoValue.innerText = servosArray[i].current;
      if (goalServo) goalServo.value = servosArray[i].goal;
      if (goalServoValue) goalServoValue.innerText = servosArray[i].goal;
